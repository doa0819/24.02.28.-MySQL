...
-- DDL 은 데이터 베이스의 스키마(=테이블)를 정의하거나 수정하는데 사용되는 
-- SQL 의 한 부분이다.
...
-- 테이블의 컬럼 설정 방법 
-- 컬럼이름 데이터타입(길이) [not null] [default value] [auto_increment] - 자동적으로 증가하는 colum_contraint;
-- [] 안에 있는 거는 써도 되고 안써도 된다.
...
create table if not exists tb1(
    ...
    fk int,
    col1 varbinary(255),
    -- check 를 할건데 오로지 Y,N 만 체크를 할것이다.
    check(col1 in ('Y', 'N'))
)ENGINE=INNODB;
...
-- 열 이름 및 데이터 형식 변경
-- alter table 테이블명 change column 기존컬럼면 바꿀컬럼면 컬럼정의
...
-- 0으로 채워진 date 컬럼이 존대하면 안 되기 때문이다.
